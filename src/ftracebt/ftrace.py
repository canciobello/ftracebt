...
class ReadBuffer:
    ...
    def is_empty(self):
        lines = self.complete_read_nc(self.trace_file)
    ...
    def get_entries_noheader_nc(self, trace_filename, nr_entries=None):
        ...
        entries_to_ignore = 0
        nr_lines = len(lines)
        while lines[entries_to_ignore].find(
                WriteBuffer.head_entry_beginning) == -1:
            entries_to_ignore += 1
            if entries_to_ignore == nr_lines:
                raise ParsingBufferHeadError(
                    "Error while trying to ignore the buffer header. No first entry pattern found."
                )
...
class FtraceManager:
    ...
    def clear_buffer(self):
        Helpers.write2file(self.trace_file, ' ')
...
class Check:
    ...
    def marker_page(buffer_content, entries_per_page, cpu, page_id):
        if len(buffer_content) < entries_per_page:
            raise CheckingMarkerPagesError(
                'Not enough entries. CPU: {} PAGE_ID: {} Entries expected: {} Entries in this page: {}'
                .format(cpu, page_id, entries_per_page, len(buffer_content)))
        if buffer_content[0].find(
                WriteBuffer.generate_page_header_entry(page_id)) == -1:
            raise CheckingMarkerPagesError(
                'First line do not match. PAGE_ID: {} Line: {}'.format(
                    page_id, buffer_content[0]))
        for entry_nr in range(1, entries_per_page):
            if buffer_content[entry_nr].find(
                    WriteBuffer.generate_entry(cpu, page_id, entry_nr)) == -1:
                raise CheckingMarkerPagesError(
                    'Line do not match. CPU: {} PAGE_ID: {} ENTRY: {}. Line: {}'
                    .format(cpu, page_id, entry_nr, buffer_content[entry_nr]))
        return (entries_per_page, page_id + 1)

    @staticmethod
    def exact_marker_pages(content_no_header,
                           entries_per_page,
                           cpu,
                           nr_pages,
                           first_page_id=1,
        ...
        if extra_entries:
            raise NotImplementedError(
                'Parameter "extra_entries" is not supported yet.')
        next_page_id = first_page_id
        for i in range(1, nr_pages + 1):
            entries, next_page_id = Check.marker_page(content_no_header[begin:],
                                                      entries_per_page, cpu,
                                                      next_page_id)
            begin += entries
        if content_no_header[begin:]:
            raise CheckingMarkerPagesError(
                'There is extra entries. First extra entry: {}'.format(
                    content_no_header[begin]))
        return True

    @staticmethod
    def _get_nr_pages_from(config, writer_name):
        nr_pages_to_fillup_buffer = config['nr_pages_to_fillup_buffer']
        writer_names_nr_pages = {
            'write_one_page': 1,
            'write_two_pages': 2,
            'write_three_pages': 3,
            'write_four_pages': 4,
            'fillup_buffer': nr_pages_to_fillup_buffer,
            'fillup_plus_one_page': nr_pages_to_fillup_buffer + 1,
            'fillup_plus_two_page': nr_pages_to_fillup_buffer + 2,
            'fillup_plus_three_page': nr_pages_to_fillup_buffer + 3,
            'fillup_plus_four_page': nr_pages_to_fillup_buffer + 4
        }
    ...
    @staticmethod
    def per_cpu_content(config, writer_name, content_no_header, cpus_to_use,
                        entries_per_page):
        for cpu in cpus_to_use:
            filtered_content = [
                x for x in content_no_header
                if x.find(' [{:03}] '.format(cpu)) != -1
            ]
            pages_written = Check._get_nr_pages_from(config, writer_name)
            nr_pages, first_page_id = Check.get_nr_pages_and_first_page_id(
                config, pages_written)
            Check.exact_marker_pages(filtered_content, entries_per_page, cpu,
                                     nr_pages, first_page_id)
    ...
    def merged_buffers(content_no_header):
        ...
        for line in content_no_header:
            if line.find(' buffer started ####') != -1:
                continue
            try:
                current_time = list(
                    int(x) for x in line.split()[3].strip(':').split('.'))
            except Exception as err:
                raise CheckingEntriesOrderError(
                    'Error while trying to get time stamp of line: {}'.format(
                        line))
            if current_time < prev_time:
                raise CheckingEntriesOrderError(
                    'Entries out of order. Entry x: {}. Entry x+1: {}'.format(
                        prev_line, line))
            prev_line = line
    ...
    def content_trace_files(trace_content, persistent_content):
        ...
        for i1, trace_line in enumerate(trace_content):
            try:
                if trace_line.find('# entries-in-buffer/entries-written: '
                                  ) != -1 and i1 == 2:
                    i2 += 1
                    continue
                if trace_line.find(' buffer started ####') != -1:
                    continue
                if trace_line != persistent_content[i2]:
                    raise CompareTraceFiles(
                        'Error while comparing trace and persistent content. A line doesn\'t match. Line index at trace content: {}. Line index at persistent content: {}. Line of trace content: "{}". Line of persistent content: "{}".'
                        .format(i1, i2, trace_line, persistent_content[i2]))
                i2 += 1
            except IndexError:
                raise CompareTraceFiles(
                    'Error while comparing trace content and persistent content. Trace content is longer than persistent content. First extra line index: {}. Line content: "{}"'
                    .format(i1, trace_line))
        if len(persistent_content) > i2:
            raise CompareTraceFiles(
                'Error while comparing trace content and and persistent content. Persistent content is longer than trace content. First extra line index: {}. Line content: "{}"'
                .format(i2, persistent_content[i2]))
